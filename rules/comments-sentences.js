...
/*
 * Base schema body for defining the basic pattern, ignorePattern,
...
/**
 * Creates a regular expression for each pattern defined in the rule
 * options.
 *
 ...
 * @returns {void}
 */
function createRegExpForPatterns(normalizedOptions) {
    const ignorePatternStr = normalizedOptions.ignorePattern;
    if (ignorePatternStr) {
        const regExp = RegExp(`^\\s*(?:${ignorePatternStr})`);

        normalizedOptions.ignorePatternRegExp = regExp;
    }
}
...
// meta: {
//     docs: {
//         description: 'enforce that comments are correct sentences (basically starting with an upper case letter and ending with a dot',
...
module.exports = function(context) {
    const normalizedOptions = getNormalizedOptions(context.options[1]);
    const sourceCode = context.getSourceCode();

    createRegExpForPatterns(normalizedOptions);
    ...
    function isCommentValid(comment, options) {
        ...
        for (var i = 0, len = lines.length; i < len; i++) {
            ...
            if (options.ignorePatternRegExp && options.ignorePatternRegExp.test(line)) {
                continue;
            }
            ...
            // Is the initial word character a letter?
            ...
            isEndingByPunctuation = PUNCTUATION.test(line);
        }

        if ((isIgnored && !wasEndingByPunctuation) || (!isIgnored && !isEndingByPunctuation)) {
